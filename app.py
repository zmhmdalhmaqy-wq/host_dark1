...
def api_logout():
    ...
    if username:
        print(f"👤 تسجيل خروج المستخدم: {username}")
...
def server_action(folder, action):
    ...
    if action == "start":
        ...
        return delete_server_action(folder, srv)

    return jsonify({"success": False})

def delete_server_action(folder, srv):
    """دالة منفصلة لحذف السيرفر"""
    print(f"🗑️ جاري حذف السيرفر: {folder}")
    ...
    if srv.get("pid"):
        try:
            ...
            for child in p.children(recursive=True):
                child.kill()
            p.kill()
            print(f"✅ تم إيقاف العملية {srv['pid']}")
        except Exception as e:
            print(f"⚠️ خطأ في إيقاف العملية: {e}")
    ...
    if os.path.exists(srv["path"]):
        try:
            ...
            try:
                ...
                print(f"✅ تم حذف المجلد باستخدام rm")
            except:
                pass
...
def delete_files(folder):
    ...
    print(f"🗑️ جاري حذف {len(names)} ملف من السيرفر {folder}")
...
def upload_files(folder):
    ...
    for f in files:
        try:
            ...
            save_path = os.path.join(srv["path"], safe_name)
            f.save(save_path)
            ...
            if safe_name.lower().endswith('.zip'):
                try:
                    ...
                    os.remove(save_path)
                    print(f"✅ تم فك ضغط الملف: {safe_name}")
                except Exception as e:
                    print(f"⚠️ خطأ في فك الضغط: {e}")
            uploaded += 1
        except Exception as e:
...
@app.route('/api/list-installed-packages', methods=['GET'])
def list_installed_packages():
    """عرض قائمة المكتبات المثبتة"""
    if "username" not in session:
        return jsonify({"success": False}), 401
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return jsonify({"success": False, "message": "فشل في جلب القائمة"})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)})
